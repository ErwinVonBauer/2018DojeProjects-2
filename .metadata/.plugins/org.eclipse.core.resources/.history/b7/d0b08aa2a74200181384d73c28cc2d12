 ...
 * @author cielo
 * @See nhn.husky.SE2M_Configuration @ 팝업 마크업은 SimplePhotoUpload.html과
 *      SimplePhotoUpload_html5.html이 있습니다.
 */
var textAreaName = "";
window.onload = function() {
	textAreaName = getSEPageName();
}
// 쿼리 스트링 받는 곳
function getSEPageName() {
	var paramOb = getQueryString();
	var name = paramOb.textAreaName;
	return name;
}
...
nhn.husky.SE2M_AttachQuickPhoto = jindo
		.$Class({
			 ...
			 * nhn.husky.SE2M_AttachQuickFPhoto.prototype.makePopupURL로 덮어써서
			 * 사용하시면 됨.
			 */
			makePopupURL : function() {
				var sPopupUrl = "/makePopup?textAreaName=" + textAreaName;
				return sPopupUrl;
			},
			...
			$ON_SET_PHOTO : function(aPhotoData) {
				var sContents, aPhotoInfo, htData;
				...
				try {
					...
					this.oApp.exec("PASTE_HTML", [ sContents ]); // 위즐 첨부 파일
					// 부분 확인
				} catch (e) {
